...
// Renderiza uma linha de usuário com avatar e info
function userLine(u, actionsHTML = '') {
    const xp = typeof u.xp === 'number' ? u.xp : (u.xp?.total ?? 0);
    const avatarSrc = u.avatarUrl || '/public/avatares/avatar.jpg';
    return `
...
// Autenticação e Sincronização em tempo real do documento do usuário
onAuthStateChanged(auth, async (user) => {
    ...
    // Configura a sincronização em tempo real do documento do usuário
    onSnapshot(userRef(user.uid), async (snap) => {
        if (!snap.exists()) {
            // Se o documento não existir, crie um documento inicial para evitar erros.
            await updateDoc(userRef(user.uid), {
                apelido: user.email.split('@')[0], // Apelido inicial
                email: user.email,
                avatarUrl: '/public/avatares/avatar.jpg',
                xp: 0
            });
            // Tente carregar novamente após a criação do documento
...
// Oculta o overlay de carregamento após o DOM ser carregado
